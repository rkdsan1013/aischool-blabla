...
export async function generateSentenceQuestionsRaw(
  ...
  const prompt = [
    ...
    "",
    `--- [지시] ---`,
    `1. 사용자의 수준(\`${normalizedLevel}\`)에 맞는 문장 순서 맞추기 문제 ${QUESTION_COUNT}개를 출제하세요.`,
    ...
    `7. **(다양성 규칙)** ${QUESTION_COUNT}개의 문제는 서로 중복되지 않아야 하며, 다양한 문법 구조(평서문, 의문문 등)를 골고루 선정해야 합니다.`,
    ...
    `- 출력은 반드시 유효한 JSON이어야 하며, 배열 길이는 정확히 ${QUESTION_COUNT}개의 문제 객체여야 합니다.`,
