...
export default function VoiceRoomDetail(): React.ReactElement {
  ...
  // Web Speech API recognition (local)
  ...
  // transcript id set to avoid duplicates
  ...
  const sendTranscriptToServer = useCallback(
    async (item: {
      ...
      if (!roomId) {
        console.warn("No roomId, cannot send transcript to server");
        return;
      }
      try {
        ...
        console.warn("Failed to send transcript to backend:", e);
      }
    },
    [roomId, profile]
  );

  /* ------------------ local recognition helpers ------------------ */
  const startLocalRecognition = useCallback(() => {
    const w = window as unknown as SafariWindow;
    const RecognitionClass =
      (window as any).SpeechRecognition || (w as any).webkitSpeechRecognition;
    ...
    try {
      ...
      recognition.onend = () => {
        ...
        // 자동 재시작 (권한/환경에 따라 실패할 수 있음)
        if (!isMuted && (socketRef.current?.connected ?? true)) {
          setTimeout(() => {
            try {
              recognition.start();
            } catch {
      ...
      recognition.onresult = (event: any) => {
        ...
        // final 처리: 서버로 전송(선택) 및 UI에 확정 텍스트 추가
        if (finalTranscript && finalTranscript.trim().length > 0) {
          ...
          // remove interim and append final
          setTranscript((prev) => {
          ...
          // 서버에 최종 자막 전송(소켓 + REST)
          socketRef.current?.emit("local_transcript", finalItem);
      ...
      console.warn("Failed to initialize SpeechRecog", e);
      recognitionRef.current = null;
    }
  }, [isMuted, sendTranscriptToServer]);
  ...
  /* ------------------ initialize room / socket / recorder ------------------ */
  useEffect(() => {
    if (!roomId) return;
    ...
    const initRoom = async () => {
      try {
        ...
        // MediaRecorder 설정 및 시작 (핸들러 등록 후 시작)
        try {
          ...
          recorder.ondataavailable = async (ev: BlobEvent) => {
            ...
            // muted 상태이거나 소켓 연결이 없으면 전송하지 않음
            if (isMuted || !socketRef.current || !socketRef.current.connected)
              return;

            try {
              // 서버가 (meta, chunk) 형태로 받도록 구현되어 있어야 함
              socketRef.current.emit(
                "audio_chunk",
                { roomId, mimeType: recorderMimeRef.current },
                ev.data
              );
            } catch (e) {
          ...
          console.warn("MediaRecorder not available or failed to start:", e);
        }

        // 로컬 recognition 시작 (있으면)
    ...
    return () => {
      ...
      participantAnalysers.current.clear();
  ...
  /* ------------------ transcript helpers & UI interactions ------------------ */
  useEffect(() => {
    setTranscript([]); // 초기화: 더미 데이터 제거
    transcriptIdsRef.current.clear();
    ...
    if (audioTrack) {
      ...
      try {
        if (rec) {
          ...
          if (newMuted) stopLocalRecognition();
  ...
  const handleSend = () => {
    ...
    // add locally and send
    addOrUpdateTranscript(newTranscript);
    setInputText("");
    socketRef.current?.emit("local_transcript", newTranscript);
